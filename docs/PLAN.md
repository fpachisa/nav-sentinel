...
| `BreakCategory` | A closed enum cannot route a second process | `handles_capabilities: list[str]`, namespaced (`nav.fx_rate`, `ta.subscription_in_transit`). `coverage()` takes its namespace from registered packs; `fleet_cli.py` stops calling `.value`. |
