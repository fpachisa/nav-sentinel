...
| Agent Gateway | Agent Gateway | Single enforcement point for the ten policies below. Subject resolved from the bound identity; tools resolved from a frozen catalogue. |
...
| `P-010-CAPABILITY-ROUTING` | Whether any published agent is authorised to investigate this capability at all — the prior question to P-001's "may this agent call that tool", and the one with the more dangerous wrong answer: handing an unroutable break to whichever agent looks closest returns a confident, wrong root cause with real citations attached and an audit trail claiming a specialist established it. Recorded on **both** outcomes, so the log names the specialist that was authorised rather than leaving it to be inferred from the tool calls that followed. Added because the most interesting thing the registry does — decline to route — was visible only as a field on a rewritable case document, which is the failure P-008's own rationale warns about. |
...
Lint, the diagram geometry checks, then **1039 invariant tests** in about five seconds. These are
...
`investigate()` function, the same gateway, the same ten policies — with a manifest and prompt
...
| Agent Gateway policy enforcement | works, within a stated boundary | All ten policies resolve from frozen registry models and the bound identity; approval minting sits behind an object the agent runtime never holds. Bypass tests: `TestCatalogueIntegrity`, `TestDataScopeEnforcement`, `TestIdentityCannotBeForged`, `TestApprovalReferencesAreResolved`. **In-process memory is not a trust boundary** — code executing inside the runtime can read module internals. What is closed is everything reachable by an agent emitting tool-call data. |
...
| Cloud Run deployment | works, and browsable | Revision `nav-sentinel-00028-d4g`, `us-central1`, running as `nav-runtime`. **Open the desk at [/app](https://nav-sentinel-rwkxhtvoeq-uc.a.run.app/app) and sign in with Google.** Ingress is public *and* the deploy script refuses to open it unless both an OAuth client id and an analyst table are configured — so there is no setting in which the service is reachable by anyone and authenticates nobody. The routes that do work still refuse without a session: `/console`, `/cycle` and `/selftest` return 401, which matters because opening ingress would otherwise have published an endpoint that *runs a reconciliation*. `/readyz` names its store and refuses to report ready if a deployment asked for Firestore and got memory. Evidence: [docs/evidence/S7a-cloud-run.md](docs/evidence/S7a-cloud-run.md) and [S14-deployed.md](docs/evidence/S14-deployed.md), [S15-signin.md](docs/evidence/S15-signin.md) |
